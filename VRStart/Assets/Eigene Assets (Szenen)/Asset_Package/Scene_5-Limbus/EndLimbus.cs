...
public class EndLimbus : MonoBehaviour {
	...
	void Update(){		
		...
		if(GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).normalizedTime > 0.3 && !GetComponent<Animator>().IsInTransition(0)) {Kreisel.GetComponent<KreiselScript>().KreiselFaellt();}
		if(GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).normalizedTime < 0.3 && !GetComponent<Animator>().IsInTransition(0)) {Kreisel.GetComponent<KreiselScript>().KreiselAufrecht();}
	}
